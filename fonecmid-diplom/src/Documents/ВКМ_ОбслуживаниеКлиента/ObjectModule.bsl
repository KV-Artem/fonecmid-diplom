...
Процедура ОбработкаПроведения(Отказ,Режим)
	...
	Запрос.Текст =
		...
		|	ЕстьNull(ВКМ_ОбслуживаниеКлиента.Договор.ВКМ_СтоимостьЧасаРаботы,0) КАК СтоимостьЧасаРаботы
		|ИЗ
	...
	ПроцентОтРабот = ВКМ_ПроцентОтРаботыДляСотрудникаВызовСервера.ПроцентОтРаботыДляСотрудника(Специалист,Дата);
	
	Если ПроцентОтРабот = Неопределено Тогда
		Отказ = Истина;
		ОбщегоНазначения.СообщитьПользователю("Для выбранного специалиста не установлен процент от выполненных заказов.",,,,Отказ);	
	Иначе
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// Если документ записывается в первый раз в базу	
